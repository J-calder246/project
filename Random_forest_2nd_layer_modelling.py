...
model_RF_fm = joblib.load("models/Fannie_mae_RF.pkl")
scaler_fm = joblib.load("models/FMscaler.pkl")
model_RF_1st = joblib.load("models/Applications_RF.pkl")
...
predictions = model_RF_1st.predict(X)

probability = model_RF_1st.predict_proba(X)[:, 1]   #probability of delinquency shows us how certain the models are 
print(predictions
      )
print(probability)
...
predictions = model_RF_fm.predict(X_MS)

probability = model_RF_fm.predict_proba(X_MS)[:, 1]   #probability of delinquency
print(predictions
      )
print(probability)
